	          ...
	          <tbody>
            	@foreach($users as $user)
            		<tr>
            			...
            			<td>{{$user->email}}</td>
	            		<td>
			              	<a href="{{ route('users').'/'.$user->id.'/edit' }}" title="ubah"><spane class="badge badge-success">edit</span></a><form method="post" action="{{ route('users').'/'.$user->id }}" style="display:inline;">
            					@method('DELETE')
            					@csrf
